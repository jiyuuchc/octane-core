...
public class Basdi {
    ...
    static final double DEFAULT_DRIFT_RESOLUTION = 16; // nm
    ...
    static final double DEFAULT_CREEP_PROBABILITY = 0.001; // per frame
    static final int DEFAULT_MAX_ITERATIONS = 5;
    static final double DEFAULT_CONVERGENCE_EPS = 0.001; // in high-res pixels

    static final double DARK_NOISE = 0.01; // per resolution pixel per frame
    static final double ANNEAL_SCALE_START = 2.4;
    static final double ANNEAL_SCALE_END = 0.4;
    ...
    public Basdi() {
        this(DEFAULT_MAX_DRIFT, DEFAULT_DRIFT_RESOLUTION,
                // DEFAULT_LOCALIZATION_SIGMA,
                DEFAULT_DRIFT_SIGMA, DEFAULT_CREEP_PROBABILITY, DEFAULT_MAX_ITERATIONS);
    }
    ...
    public Basdi(double maxDistance, double pixelSize, double driftSigma, double creepProbability, int maxIterations) {
        ...
        if (maxDistance <= 0 || maxIterations <= 0 || pixelSize <= 0 || maxIterations <= 0 || creepProbability < 0) {
    ...
    public void estimate(LocalizationDataset data, int numOfKeyFrames) {
    ...
    protected void correct(LocalizationDataset data) {
        LocalizationImage locs = new LocalizationImage(data);
        double[] xcol = locs.getData(locs.xCol);
        double[] ycol = locs.getData(locs.yCol);
    ...
    public void optimizeWithAnnealing() {
        ...
        while (scale >= ANNEAL_SCALE_END) {
            ...
            Arrays.fill(annealingFilter, 1.0 / fs);
    ...
    protected void updateTheta(RectangularDoubleImage theta, RectangularDoubleImage[] marginalProb, int[][][] data) {
        ...
        //calc mean shift on first frame
        double cx = 0, cy = 0;
        for (int idx = 0; idx < gn.getLength(); idx++) {
            cx += gn.getValueVector()[idx] * gn.getXCordinate(idx);
            cy += gn.getValueVector()[idx] * gn.getYCordinate(idx);
        ...
        logger.info("cx: {} - cy: {}", icx, icy);

        // expand the image by 2 x maxShift, so that the convolution will not be out of
        // bound
        double[] values = new double[(theta.height + 2 * maxShift) * (theta.width + 2 * maxShift)];
        RectangularDoubleImage theta0 = new RectangularDoubleImage(values, theta.width + 2 * maxShift,
                theta.x0 - maxShift, theta.y0 - maxShift);
        IntStream.range(0, data.length).parallel().forEach(k -> {
            RectangularDoubleImage img = new RectangularDoubleImage(new double[theta0.getLength()], theta0.width,
                    theta0.x0, theta0.y0);
        ...
        RectangularDoubleImage thetaNew = new RectangularDoubleImage(theta0, theta.x0 + icx, theta.y0 + icy,
                theta.width, theta.height);
    ...
    protected RectangularDoubleImage[] exy(RectangularDoubleImage theta, int[][][] data) {
        ...
        // initialize
        int w = theta.width + maxShift * 2;
        int h = theta.height + maxShift * 2;
        RectangularDoubleImage tmp = new RectangularDoubleImage(new double[w*h], w, -maxShift, -maxShift);
        tmp.copyFrom(theta);
        final RectangularDoubleImage logTheta = ImageFilters.symmetricFilter(annealingFilter, tmp);
     ...
     */
    protected RectangularDoubleImage exyf2(final RectangularDoubleImage logTheta, final int[][] o) {

        int matrixSize = maxShift * 2 + 1;
        RectangularDoubleImage e0 = new RectangularDoubleImage(new double[matrixSize * matrixSize], matrixSize,
                -maxShift, -maxShift);
    ...
    /*
     * % forward_backward alogorithm for computing marginal probability % of
     * Markovian process
     */
    ...
    void showImage(RectangularDoubleImage img) {
        for (int y = img.y0; y < img.y0 + img.height; y++) {
            for (int x = img.x0; x < img.x0 + img.width; x++ ) {
                System.out.print((int) FastMath.round(img.getValueAtCoordinate(x, y) * img.getLength()));
                System.out.print(",");
            }
            System.out.println();
        }
