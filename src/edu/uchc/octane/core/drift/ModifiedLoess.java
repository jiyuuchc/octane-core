...
public class ModifiedLoess
	...
	public final PolynomialSplineFunction interpolate(double[] xval,
        ...
        double [] newX = new double[xval.length];
        double [] newSmoothed = new double[xval.length];
        newX[0] = xval[0];
        newSmoothed[0] = smoothed[0];
        int idx = 1;
        for(int i = 1; i < xval.length; i++){
            if(xval[i] != xval[i-1]){
                newX[idx] = xval[i];
                newSmoothed[idx] = smoothed[i];
                idx ++;
            }
        }
        xval = new double[idx];
        smoothed = new double[idx];
        System.arraycopy(newX, 0, xval, 0, idx);
        System.arraycopy(newSmoothed, 0, smoothed, 0, idx);
